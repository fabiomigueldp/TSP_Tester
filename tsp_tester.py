#!/usr/bin/env python3
"""
tsp_tester.py - Professional Test Tool for the Trading-Signal-Processor (v3.0 Updated).

This application is a comprehensive load and functional testing utility,
designed specifically for the Trading-Signal-Processor microservice.

Features:
- Sends signals with pre-defined tickers to test approval and rejection logic.
- Smooth, distributed request sending pattern over time.
- Controls the ratio between signals that should be approved vs. rejected.
- Dynamic payload templating with Faker for realistic data.
- Controls for RPS, ramp-up period, and test duration.
- Real-time monitoring of the application's internal state via API polling.
- Integrity checks by comparing data sent vs. data processed by the application.
- Live graphs of RPS, latency, queue sizes, and signal counts.
- A control panel to interact with all of the application's admin endpoints in real-time.
- Test profile management (save/load).
- Detailed results export in CSV format.
- Headless/CLI mode for automated testing.
"""
...
class AsyncWorker:
    ...
    def __init__(self, config: Dict, output_queue: queue.Queue, shutdown_event: threading.Event):
        self.config = config
        self.config_lock = threading.Lock()  # For thread-safe access
        self.output_queue = output_queue
        self.shutdown_event = shutdown_event
        self.test_start_time = 0

    def update_config(self, new_config: Dict):
        """Dynamically updates the configuration during the test."""
        with self.config_lock:
            # Update only fields that can be changed in real-time
        ...
        async with httpx.AsyncClient() as client:
            while not self.shutdown_event.is_set():
                ...
                for i, ticker in enumerate(batch):
                    ...
                    # Spreading logic to distribute requests over the second
                ...
                if remaining_sleep > 0:
                    # Sleep in smaller chunks for faster response to shutdown
...
class TestController:
    ...
    def __init__(self, app: 'ProStressTesterApp'):
        ...
        self.duration_timer_id = None
        self.reset_stats()

    def reset_stats(self):
        self.start_time = 0
        self.total_sent = 0
        self.sent_approved = 0
        self.sent_rejected = 0
        self.response_success = 0
        self.response_fail = 0
        self.error_counter = Counter()
        self.latencies = []
        self.last_app_status = {}
        if hasattr(self, 'app') and self.app:
            self.app.clear_graphs()
            self.app.clear_output_log()
            self.app.update_app_status_display({})
    ...
    def start_test(self):
        ...
        self.poller_thread = threading.Thread(target=poller.run, daemon=True)
    ...
    def stop_test(self):
        ...
        if self.duration_timer_id:
            self.app.master.after_cancel(self.duration_timer_id)
            self.duration_timer_id = None
        self.state = "STOPPING"
        self.app.set_control_state(self.state)
        ...
        if self.shutdown_event:
            ...
            # Schedule a check for thread completion to avoid blocking the GUI
            self.app.master.after(100, self._check_stop_completion)

    def _check_stop_completion(self):
        """Checks if threads have finished without blocking the GUI."""
        ...
        if threads_finished:
            ...
            # Threads still running, check again in 100ms
            self.app.master.after(100, self._check_stop_completion)
        ...
        try:
            while not self.output_queue.empty():
                ...
                if msg_type == "log": self.app.log_output(payload)
                    ...
                    self.last_app_status = payload
                    self.app.update_app_status_display(payload)
                elif msg_type == "test_finished":
                    # Only change state if not already being stopped manually
                    if self.state != "STOPPING":
                        if self.poller_thread and self.shutdown_event:
                            self.shutdown_event.set()
                            self.poller_thread.join(timeout=1)
                        self.state = "FINISHED"
                        self.app.set_control_state(self.state)
    ...
    def update_stats_display(self, *args):
        ...
        # Safely get metrics from the nested structure
        ...
        proc_q_size = app_metrics.get('processing_queue_size', 0)
        appr_q_size = app_metrics.get('approved_queue_size', 0)
        
        webhook_metrics = self.last_app_status.get('webhook_rate_limiter', {})
        webhook_tokens = webhook_metrics.get('tokens_available', 0)
        webhook_req_min = webhook_metrics.get('requests_in_last_minute', 0)
        ...
        self.app.add_graph_data(
            t=elapsed_time, rps=rps, latency_ms=avg_lat * 1000, 
            proc_q_size=proc_q_size, appr_q_size=appr_q_size,
            sent_approved=self.sent_approved, app_approved=app_approved,
            webhook_tokens=webhook_tokens, webhook_req_min=webhook_req_min
        )

        if len(self.latencies) > 5000: self.latencies = self.latencies[-2500:]

    def update_config_realtime(self, new_config: Dict):
        """Updates configuration in real-time during an active test."""
        if self.state != "RUNNING": return
    ...
    def save_config(self):
        """Saves the current test profile to a JSON file."""
        ...
        for key in ['url', 'app_status_url', 'max_concurrency']:
    ...
    def load_config(self):
        """Loads a test profile from a JSON file."""
        if not self.app: return
    ...
    def _send_admin_command(self, endpoint: str, payload: Dict):
        """Sends an administrative command to the application."""
        if not self.app: return
        config = self.app.get_current_config()
        if not config: return
            ...
            try:
                with httpx.Client(timeout=10.0) as client:
                    ...
                    self.app.master.after(0, lambda: self.app.log_output(
                        f"Admin command successful - {endpoint}: {message}"
                    ))
...
class ProStressTesterApp(ttk.Frame):
    def __init__(self, master=None, controller=None):
        ...
        self.rps_data = deque(maxlen=MAX_GRAPH_POINTS)
        self.latency_data = deque(maxlen=MAX_GRAPH_POINTS)
        self.proc_q_data = deque(maxlen=MAX_GRAPH_POINTS)
        self.appr_q_data = deque(maxlen=MAX_GRAPH_POINTS)
        self.sent_approved_data = deque(maxlen=MAX_GRAPH_POINTS)
        self.app_approved_data = deque(maxlen=MAX_GRAPH_POINTS)
        self.webhook_tokens_data = deque(maxlen=MAX_GRAPH_POINTS)
        self.webhook_req_min_data = deque(maxlen=MAX_GRAPH_POINTS)
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(label="Export Results as CSV", command=self.controller.export_results, state="disabled")
        file_menu.add_separator()
    ...
    def _create_widgets(self):
        ...
        self.runtime_tab = ttk.Frame(self.notebook, padding="10")
        self.graphs_tab = ttk.Frame(self.notebook, padding="10")
        self.admin_tab = ttk.Frame(self.notebook, padding="10")
        self.log_tab = ttk.Frame(self.notebook, padding="10")

        self.notebook.add(self.config_tab, text="Test Configuration")
        self.notebook.add(self.runtime_tab, text="Live Stats & Verification")
        self.notebook.add(self.graphs_tab, text="Live Graphs")
        self.notebook.add(self.admin_tab, text="App Controls")
    ...
    def _create_config_tab(self):
        ...
        app_frame = ttk.LabelFrame(frame, text="Target Application", padding=10)
        app_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=5)
        app_frame.columnconfigure(1, weight=1)
        ttk.Label(app_frame, text="App Host:").grid(row=0, column=0, sticky="w", padx=5)
        self.app_host_entry = ttk.Entry(app_frame)
        self.app_host_entry.insert(0, "localhost")
        self.app_host_entry.grid(row=0, column=1, sticky="ew")
        ttk.Label(app_frame, text="App Port:").grid(row=0, column=2, sticky="w", padx=5)
        self.app_port_entry = ttk.Entry(app_frame, width=10)
        self.app_port_entry.insert(0, "80")
        self.app_port_entry.grid(row=0, column=3, sticky="w")

        ticker_pane = ttk.PanedWindow(frame, orient=tk.HORIZONTAL)
        ticker_pane.grid(row=1, column=0, columnspan=2, sticky="nsew", pady=5)

        def create_ticker_frame(parent, title, default_tickers):
            f = ttk.LabelFrame(parent, text=title, padding=10)
            f.columnconfigure(0, weight=1)
            f.rowconfigure(0, weight=1)
            tree = ttk.Treeview(f, columns=("ticker",), show="headings", height=5)
            tree.heading("ticker", text="Ticker")
            tree.grid(row=0, column=0, sticky="nsew")
            for t in default_tickers: tree.insert("", "end", values=(t,))
            buttons = ttk.Frame(f)
            buttons.grid(row=0, column=1, sticky="ns", padx=5)
        ...
        ticker_pane.add(rejected_frame, weight=1)

        payload_pane = ttk.PanedWindow(frame, orient=tk.HORIZONTAL)
        payload_pane.grid(row=2, column=0, columnspan=2, sticky="nsew", pady=5)
        frame.rowconfigure(2, weight=1)
        
        payload_frame = ttk.LabelFrame(payload_pane, text="Request Body Template (use {ticker} and {faker...})", padding=10)
        self.payload_text = ScrolledText(payload_frame, height=10, wrap=tk.WORD)
        self.payload_text.insert("1.0", DEFAULT_PAYLOAD_TEMPLATE)
        ...
        self.headers_text = ScrolledText(headers_frame, height=10, wrap=tk.WORD)
        self.headers_text.insert("1.0", '{\n    "Content-Type": "application/json"\n}')
        self.headers_text.pack(fill="both", expand=True)
        
        payload_pane.add(payload_frame, weight=1)
        payload_pane.add(headers_frame, weight=1)
    ...
    def _create_runtime_tab(self):
        ...
        top_pane.grid(row=0, column=0, sticky="ew", pady=5)
        
        ctrl_frame = ttk.LabelFrame(top_pane, text="Test Controls", padding=10)
        ...
        def create_slider(parent, text, from_, to, default, row, config_key=None):
            ...
            label = ttk.Label(parent, text=str(default), width=5)
        ...
        action_frame = ttk.Frame(ctrl_frame)
        action_frame.grid(row=4, column=0, columnspan=3, pady=10)
        self.start_button = ttk.Button(action_frame, text="Start Test", command=self.controller.start_test, style="Accent.TButton")
        self.start_button.pack(side="left", padx=10)
        self.stop_button = ttk.Button(action_frame, text="Stop Test", command=self.controller.stop_test)
        self.stop_button.pack(side="left", padx=10)
        
        top_pane.add(ctrl_frame, weight=1)
        
        stats_pane = ttk.PanedWindow(frame, orient=tk.VERTICAL)
        stats_pane.grid(row=1, column=0, sticky="nsew", pady=10)
        ...
        stats_pane.add(tester_stats_frame, weight=1)
        stats_pane.add(app_stats_frame, weight=1)
        stats_pane.add(verify_stats_frame, weight=1)
        ...
        create_stat_label(app_stats_frame, "Processing Queue:", "proc_q_size", 0, 4, self.app_status_labels)
        create_stat_label(app_stats_frame, "Approved Queue:", "appr_q_size", 1, 4, self.app_status_labels)
        create_stat_label(app_stats_frame, "Finviz Engine:", "engine_status", 0, 6, self.app_status_labels)
        create_stat_label(app_stats_frame, "Webhook Limiter:", "webhook_limiter", 1, 6, self.app_status_labels)
    ...
    def _create_admin_tab(self):
        ...
        # Admin Token Frame
        token_frame = ttk.LabelFrame(frame, text="Admin Token", padding=10)
        token_frame.grid(row=0, column=0, sticky="ew", pady=5)
        token_frame.columnconfigure(1, weight=1)
        ttk.Label(token_frame, text="Token:").grid(row=0, column=0, sticky="w", padx=5)
        self.admin_token_entry = ttk.Entry(token_frame)
        self.admin_token_entry.insert(0, "your-secret-token")
        self.admin_token_entry.grid(row=0, column=1, sticky="ew")

        # Main Paned Window for Controls
        main_pane = ttk.PanedWindow(frame, orient=tk.HORIZONTAL)
        main_pane.grid(row=1, column=0, sticky="nsew", pady=5)
        frame.rowconfigure(1, weight=1)
        ...
        # --- Left Pane ---
        engine_frame = ttk.LabelFrame(left_pane, text="Finviz Engine Control", padding=10)
        engine_frame.grid(row=0, column=0, sticky="ew", pady=(0, 5))
        ...
        config_frame = ttk.LabelFrame(left_pane, text="Update Finviz Config", padding=10)
        config_frame.grid(row=1, column=0, sticky="ew", pady=5)
        config_frame.columnconfigure(1, weight=1)
        ttk.Label(config_frame, text="New URL:").grid(row=0, column=0, sticky="w")
        self.new_finviz_url_entry = ttk.Entry(config_frame)
        self.new_finviz_url_entry.grid(row=0, column=1, sticky="ew")
        ttk.Label(config_frame, text="New Top-N:").grid(row=1, column=0, sticky="w")
        self.new_top_n_entry = ttk.Entry(config_frame, width=10)
        self.new_top_n_entry.grid(row=1, column=1, sticky="w")
        ttk.Label(config_frame, text="New Refresh (s):").grid(row=2, column=0, sticky="w")
        self.new_refresh_entry = ttk.Entry(config_frame, width=10)
        self.new_refresh_entry.grid(row=2, column=1, sticky="w")
        
        def update_config_cmd():
            payload = {"token": self.admin_token_entry.get()}
            if self.new_finviz_url_entry.get(): payload['url'] = self.new_finviz_url_entry.get()
            ...
            if len(payload) > 1:
                ...
                self.log_output("Admin: No new Finviz config values entered.")
        ttk.Button(config_frame, text="Update Finviz Config", command=update_config_cmd).grid(row=3, column=0, columnspan=2, pady=5)

        metrics_frame = ttk.LabelFrame(left_pane, text="Metrics Control", padding=10)
        ...
        # --- Right Pane ---
        auth_frame = ttk.LabelFrame(right_pane, text="Finviz Elite Authentication", padding=10)
        auth_frame.grid(row=0, column=0, sticky="ew", pady=(0, 5))
        auth_frame.columnconfigure(1, weight=1)
        ttk.Label(auth_frame, text="Email:").grid(row=0, column=0, sticky="w")
        self.elite_email_entry = ttk.Entry(auth_frame)
        self.elite_email_entry.grid(row=0, column=1, sticky="ew")
        ttk.Label(auth_frame, text="Password:").grid(row=1, column=0, sticky="w")
        self.elite_pass_entry = ttk.Entry(auth_frame, show="*")
        self.elite_pass_entry.grid(row=1, column=1, sticky="ew")
        
        auth_btn_frame = ttk.Frame(auth_frame)
        auth_btn_frame.grid(row=2, column=0, columnspan=2, pady=5)
        ttk.Button(auth_btn_frame, text="Login", command=lambda: self.controller._send_admin_command('/admin/finviz-auth/login', {"token": self.admin_token_entry.get(), "email": self.elite_email_entry.get(), "password": self.elite_pass_entry.get()})).pack(side="left", padx=5)
        ttk.Button(auth_btn_frame, text="Logout", command=lambda: self.controller._send_admin_command('/admin/finviz-auth/logout', {"token": self.admin_token_entry.get()})).pack(side="left", padx=5)
        
        limiter_frame = ttk.LabelFrame(right_pane, text="Webhook Rate Limiter Control", padding=10)
        limiter_frame.grid(row=1, column=0, sticky="ew", pady=5)
        limiter_frame.columnconfigure(1, weight=1)
        
        ttk.Label(limiter_frame, text="Max Req/Min:").grid(row=0, column=0, sticky="w")
        self.limiter_max_req_entry = ttk.Entry(limiter_frame, width=10)
        self.limiter_max_req_entry.grid(row=0, column=1, sticky="w")
        self.limiter_enabled_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(limiter_frame, text="Enabled", variable=self.limiter_enabled_var).grid(row=1, column=0, columnspan=2, sticky='w')
        
        def update_limiter_cmd():
            payload = {"token": self.admin_token_entry.get()}
            if self.limiter_max_req_entry.get(): payload['max_req_per_min'] = int(self.limiter_max_req_entry.get())
            payload['enabled'] = self.limiter_enabled_var.get()
            self.controller._send_admin_command('/admin/webhook-rate-limiter/config', payload)

        ttk.Button(limiter_frame, text="Update Limiter Config", command=update_limiter_cmd).grid(row=2, column=0, columnspan=2, pady=5)

    def _create_log_tab(self):
        frame = self.log_tab
        frame.rowconfigure(0, weight=1)
        frame.columnconfigure(0, weight=1)
        self.output_text = ScrolledText(frame, state='disabled', wrap=tk.WORD, height=15)
    ...
    def set_current_config(self, config: Dict):
        ...
        self.rps_slider.set(config.get('rps', 100))
        self.ratio_slider.set(config.get('approved_ratio', 80))
        self.ramp_up_slider.set(config.get('ramp_up_s', 10))
        self.duration_slider.set(config.get('duration_s', 60))
    ...
    def set_control_state(self, state: str):
        ...
        if state == "STOPPING": self.stop_button.config(text="Stopping...", state=tk.DISABLED)

        for tab_frame in [self.config_tab]:
             for widget in tab_frame.winfo_children():
                self._set_state_recursive(widget, tk.NORMAL if is_idle else tk.DISABLED)
        
        self.rps_slider.config(state=tk.NORMAL if is_running or is_idle else tk.DISABLED)
        self.ratio_slider.config(state=tk.NORMAL if is_running or is_idle else tk.DISABLED)
        self.ramp_up_slider.config(state=tk.NORMAL if is_running or is_idle else tk.DISABLED)
        self.duration_slider.config(state=tk.NORMAL if is_idle else tk.DISABLED)
    ...
    def update_app_status_display(self, status: Dict):
        if "error" in status:
            ...
            self.app_status_labels['engine_status'][0].set(status['error'])
            return

        metrics = status.get('signal_processing', {})
        engine_info = status.get('engine', {})
        webhook_info = status.get('webhook_rate_limiter', {})
        ...
        self.app_status_labels['proc_q_size'][0].set(metrics.get('processing_queue_size', 'N/A'))
        self.app_status_labels['appr_q_size'][0].set(metrics.get('approved_queue_size', 'N/A'))
    ...
    def clear_graphs(self):
        self.time_data.clear(); self.rps_data.clear(); self.latency_data.clear()
        self.proc_q_data.clear(); self.appr_q_data.clear()
        self.sent_approved_data.clear(); self.app_approved_data.clear()
        self.webhook_tokens_data.clear(); self.webhook_req_min_data.clear()

        for ax in [self.ax1, self.ax2, self.ax3, self.ax4]: ax.clear()
        if hasattr(self, 'ax1_twin'): self.ax1_twin.clear()
        if hasattr(self, 'ax2_twin'): self.ax2_twin.clear()
        if hasattr(self, 'ax4_twin'): self.ax4_twin.clear()

        self.ax1.set_title("RPS & Latency")
        self.ax1.set_ylabel("RPS", color='tab:blue')
        self.ax1.tick_params(axis='y', labelcolor='tab:blue')
        self.ax1.grid(True, linestyle='--', alpha=0.6)
        if not hasattr(self, 'ax1_twin'): self.ax1_twin = self.ax1.twinx()
        self.ax1_twin.set_ylabel("Avg Latency (ms)", color='tab:orange')
        self.ax1_twin.tick_params(axis='y', labelcolor='tab:orange')

        self.ax2.set_title("Queue Sizes")
        self.ax2.set_ylabel("Queue Size")
        self.ax2.grid(True, linestyle='--', alpha=0.6)
        
        self.ax3.set_title("Verification: Sent vs. App Approved (Cumulative)")
        self.ax3.set_xlabel("Time (s)")
        self.ax3.set_ylabel("Total Count")
        self.ax3.grid(True)

        self.ax4.set_title("Webhook Rate Limiter")
        self.ax4.set_xlabel("Time (s)")
        self.ax4.set_ylabel("Tokens Available", color='tab:purple')
        self.ax4.tick_params(axis='y', labelcolor='tab:purple')
        self.ax4.grid(True, linestyle='--', alpha=0.6)
        if not hasattr(self, 'ax4_twin'): self.ax4_twin = self.ax4.twinx()
        self.ax4_twin.set_ylabel("Requests / Min", color='tab:red')
        self.ax4_twin.tick_params(axis='y', labelcolor='tab:red')
        
        self.fig.tight_layout()
        self.canvas.draw()

    def add_graph_data(self, t, rps, latency_ms, proc_q_size, appr_q_size, sent_approved, app_approved, webhook_tokens, webhook_req_min):
        self.time_data.append(t); self.rps_data.append(rps); self.latency_data.append(latency_ms)
        self.proc_q_data.append(proc_q_size); self.appr_q_data.append(appr_q_size)
        self.sent_approved_data.append(sent_approved); self.app_approved_data.append(app_approved)
        self.webhook_tokens_data.append(webhook_tokens); self.webhook_req_min_data.append(webhook_req_min)

        self.ax1.clear(); self.ax1_twin.clear()
        self.ax1.plot(self.time_data, self.rps_data, color='tab:blue', label='RPS')
        self.ax1_twin.plot(self.time_data, self.latency_data, color='tab:orange', label='Latency (ms)')
        self.ax1.set_title("RPS & Latency"); self.ax1.set_ylabel("RPS", color='tab:blue'); self.ax1_twin.set_ylabel("Avg Latency (ms)", color='tab:orange'); self.ax1.grid(True, linestyle='--', alpha=0.6)

        self.ax2.clear()
        self.ax2.plot(self.time_data, self.proc_q_data, color='cyan', label='Processing Queue')
        self.ax2.plot(self.time_data, self.appr_q_data, color='magenta', label='Approved Queue')
        self.ax2.set_title("Queue Sizes"); self.ax2.set_ylabel("Queue Size"); self.ax2.grid(True, linestyle='--', alpha=0.6); self.ax2.legend(loc='upper left', fontsize='small')

        self.ax3.clear()
        self.ax3.plot(self.time_data, self.sent_approved_data, color='cyan', label='Tester Sent "Approved"', linestyle='--')
        self.ax3.plot(self.time_data, self.app_approved_data, color='lime', label='App Processed "Approved"')
        self.ax3.set_title("Verification: Sent vs. App Approved (Cumulative)"); self.ax3.set_xlabel("Time (s)"); self.ax3.set_ylabel("Total Count"); self.ax3.legend(loc='upper left', fontsize='small'); self.ax3.grid(True)
        
        self.ax4.clear(); self.ax4_twin.clear()
        self.ax4.plot(self.time_data, self.webhook_tokens_data, color='tab:purple', label='Tokens Available')
        self.ax4_twin.plot(self.time_data, self.webhook_req_min_data, color='tab:red', label='Requests/Min')
        self.ax4.set_title("Webhook Rate Limiter"); self.ax4.set_xlabel("Time (s)"); self.ax4.set_ylabel("Tokens Available", color='tab:purple'); self.ax4_twin.set_ylabel("Requests / Min", color='tab:red'); self.ax4.grid(True, linestyle='--', alpha=0.6)
...
def run_headless(args):
    ...
    config['app_host'] = args.host or config.get('app_host', 'localhost')
    config['app_port'] = args.port or config.get('app_port', 80)
    config['url'] = f"http://{config['app_host']}:{config['app_port']}/webhook/in"
    config['app_status_url'] = f"http://{config['app_host']}:{config['app_port']}/admin/system-info"
    config['rps'] = args.rps or config.get('rps', 100)
    config['duration_s'] = args.duration or config.get('duration_s', 60)
    config['max_concurrency'] = 500 # Default for headless

    output_queue = queue.Queue()
    shutdown_event = threading.Event()
    worker = AsyncWorker(config, output_queue, shutdown_event)
    worker_thread = threading.Thread(target=worker.run, daemon=True)
    worker_thread.start()
    
    poller = AppStatusPoller(config, output_queue, shutdown_event)
    ...
    start_time = time.monotonic()
    sent_approved, sent_rejected, resp_ok, resp_err, total_sent = 0, 0, 0, 0, 0
    last_app_status = {}
    ...
    while not output_queue.empty():
        ...
        elif msg['type'] == 'app_status':
            last_app_status = msg['payload']
    ...
    print(f"    - 'Approved' Tickers: {sent_approved}")
    print(f"    - 'Rejected' Tickers: {sent_rejected}")
    print(f"  HTTP Responses OK: {resp_ok} ({resp_ok/total_sent*100:.2f}%)")
    print(f"  HTTP Responses Error: {resp_err} ({resp_err/total_sent*100:.2f}%)")

    print(f"\n[Application Stats (Last Poll)]")
    app_metrics = last_app_status.get('signal_processing', {})
    app_approved = app_metrics.get('signals_approved', 0)
    app_rejected = app_metrics.get('signals_rejected', 0)
    print(f"  App Signals Received: {app_metrics.get('signals_received', 0)}")
    print(f"  App Signals Approved: {app_approved}")
    print(f"  App Signals Rejected: {app_rejected}")

    print(f"\n[Verification]")
    print(f"  Approved Mismatch: {sent_approved - app_approved}")
    print(f"  Rejected Mismatch: {sent_rejected - app_rejected}")
...
def main():
    ...
    parser.add_argument('--host', type=str, help='Override app host from config file.')
    parser.add_argument('--port', type=int, help='Override app port from config file.')
    parser.add_argument('--rps', type=int, help='Override RPS from config file.')
    parser.add_argument('--duration', type=int, help='Override test duration (seconds) from config file.')
    args = parser.parse_args()

    if args.cli:
        if not args.config:
            parser.error("--config file is required for CLI mode.")
        run_headless(args)
    else:
        root = tk.Tk()
        ...
        style.configure('Mismatched.TLabel', foreground='red', font='-weight bold')
